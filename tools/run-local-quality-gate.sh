...
"
start_lane "frontend unit tests" bash -c "
  set -Eeuo pipefail
  cd apps/frontend
  pnpm vitest --run --maxWorkers=${lane_workers}
"
start_lane "browser end-to-end tests" bash -c "
  set -Eeuo pipefail
  cd apps/frontend
  pnpm exec playwright test --workers=${lane_workers}
"
