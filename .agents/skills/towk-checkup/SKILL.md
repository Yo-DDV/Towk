---
name: "towk-checkup"
description: "Run a documentation-focused maintenance checkup of the Towk codebase. Fans out to /fdr, /adr, and /towk-architecture, then compiles a single consolidated report. Always propose-only \u2014 no changes applied without explicit user approval. At the end, points the human at other maintenance skills (/update-project-dependencies, /towk-security-review) they may want to run themselves."
---
...
- **`/towk-architecture`** — refresh `docs/ARCHITECTURE.md` against the current state of core services, projections, EVT event tokens, EVT/live subject patterns, current NATS resources, ConnectRPC endpoints, and realtime websocket behavior. Report drift; propose updates.
...
5. Then a final **Other skills you may want to run** section that lists, as plain pointers (no invocation):
   ...
   - `/towk-security-review` — multi-agent security audit
...
- **Don't invoke `/update-project-dependencies` or `/towk-security-review`.** They're surfaced as pointers at the end of the report; the human invokes them.
