...
test.describe('Notification Bell & Page', () => {
  test('foreground non-call activity remains in the Towk notification center', async ({
    ...
    await expect(chatPage.getRoomRow('general').getByTestId('room-notification-badge')).toBeVisible(
      {
...
test.describe('Cross-device Sync', () => {
  ...
  test('dismissing a mention notification clears the room badge on other tabs and survives reload', async ({
    ...
    await withLoggedInServerWindow(browser!, serverURL, userA, async ({ page: page1b }) => {
      ...
      await expect(async () => {
        await expect(
          chatPage1bAfter.getRoomRow('general').getByTestId('room-notification-badge')
        ).not.toBeVisible();
