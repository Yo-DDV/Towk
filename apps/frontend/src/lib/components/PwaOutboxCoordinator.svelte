  ...
  async function flushAll(force = false): Promise<void> {
    ...
    const work = scopes.map(async (scope) => {
      ...
      if (!supportsMessageCreateIdempotency(store.serverInfo)) {
        await pwaOutbox.markUnsupported(scope);
        return;
      }
...
{#if summary.queued > 0 || summary.needsAttention > 0}
  <section
    class="pointer-events-auto fixed right-3 bottom-[calc(env(safe-area-inset-bottom,0px)+0.75rem)] z-[55] flex max-w-[min(26rem,calc(100vw-1.5rem))] surface-pop items-center gap-3 rounded-xl border border-border bg-surface-200/95 px-3 py-2 shadow-xl backdrop-blur"
    role="status"
    aria-live="polite"
  ...
  >
    ...
    <button
      type="button"
      class="btn-secondary shrink-0 soft-press btn-sm"
