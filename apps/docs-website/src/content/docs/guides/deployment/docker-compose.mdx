...
The generated `.env` also enables Towk's local operator API socket at `/tmp/chatto/operator.sock` inside the app container. Run operator commands with `docker compose exec` as the `chatto` user, and use `list --search` to find a stable user ID before mutating an account:
