...
The Towk binary includes an embedded NATS server with JetStream for storage. It expects a `chatto.toml` configuration file (you can generate one using `chatto init`), and stores all data in a `./data/` directory.

## Prerequisites

- A Towk binary built from a reviewed source commit. The [quick start](/getting-started/quick-start/) shows the pinned toolchain and build command. Stable binary releases and a Homebrew formula are not currently published.
...
ExecStart=/opt/towk/chatto run -c chatto.toml
Restart=always
RestartSec=5
