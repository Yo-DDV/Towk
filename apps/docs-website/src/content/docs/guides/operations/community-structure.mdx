...
description: Plan rooms, room groups, universal rooms, roles, permissions, and server branding for a self-hosted Towk server.
---

import { Aside, CardGrid, LinkCard } from "@astrojs/starlight/components";

Set up the shape of your Towk server before you invite a large group of members. The important choices are room layout, default access, announcement channels, roles, and the public identity of the server.
...
Universal rooms do not write one membership record per user. Towk derives effective membership from the room's `room.join` permission and active room bans. If you later disable Universal, previous explicit memberships remain intact.
...
Avoid server-scope denies unless you really want to block a capability everywhere. Towk uses deny-wins resolution, so a broad deny can be hard to reason about later.
