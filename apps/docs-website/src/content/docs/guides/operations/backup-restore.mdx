...
- **Owner-managed runtime configuration facts** — including the performance profile selected in Server administration
...
archive can restore the owner-selected performance profile, but it cannot
recreate host CPU/memory limits or external object-storage contents.
...
<Aside type="caution">
  Make sure Towk is **not running** before restoring. For embedded NATS setups, the restore command starts its own temporary NATS server. For external NATS, stop the Towk application but keep NATS running.
</Aside>
...
<Aside type="tip">
  Keep the same `core.secret_key` in `towk.toml` when restoring if you want existing bearer tokens, cookie-session handles, OAuth codes, and pending account-flow credentials to remain valid. Using a new value is a clean way to invalidate them during disaster recovery.
</Aside>
