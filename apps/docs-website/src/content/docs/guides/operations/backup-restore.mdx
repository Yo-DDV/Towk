...
Run `chatto backup` while your Towk server is running. The command connects to NATS via the client configuration in `chatto.toml`.
...
# Custom output path
chatto backup -c chatto.toml -o /mnt/backups/towk-daily.tar.gz
```
...
```bash
chatto backup -c chatto.toml --encrypt --include-keys --passphrase-file "$BACKUP_PASSPHRASE_FILE"

# Or pipe from a secrets manager
vault kv get -field=passphrase secret/towk | chatto backup -c chatto.toml --encrypt --include-keys
```
...
2. **Run the restore command**
   ...
   ```bash
   chatto restore backup.tar.gz.age -c chatto.toml --passphrase-file "$BACKUP_PASSPHRASE_FILE"
   ```
...
```bash
chatto keys export -c chatto.toml -o keys.backup --passphrase-file "$KEY_BACKUP_PASSPHRASE_FILE"

# Or pipe from a secrets manager
vault kv get -field=passphrase secret/towk | chatto keys export -c chatto.toml -o keys.backup
```
...
chatto backup -c "$CONFIG" --encrypt --include-keys --passphrase-file "$PASSPHRASE_FILE" -o "$BACKUP_FILE"
...
chatto keys export -c "$CONFIG" -o "$BACKUP_DIR/$TIMESTAMP-keys.age" --passphrase-file "$PASSPHRASE_FILE"
```
...
- If you use split data and key backups, apply the same retention window to Towk backup archives and `chatto keys export` files.
