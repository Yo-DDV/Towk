...
description: How Towk decides who can do what through roles, permissions, room overrides, and owner recovery.
---

import { Aside, LinkCard } from "@astrojs/starlight/components";

Towk's permissions system is built around two ideas: **roles** group people, and **permissions** describe capabilities. Every authenticated user has the implicit `everyone` role and may have additional roles such as `moderator`, `admin`, custom roles, or `owner`.
...
Every Towk server starts with four system roles. You cannot delete them.
...
When a user tries to do something, Towk resolves the decision like this:
...
3. **Deny-wins resolution.** For non-owners, Towk collects applicable user and role decisions at server, group, and room scope. If any applicable decision is a deny, the action is denied.
...
| `role.assign` | Assign or revoke roles for users except `owner`; owner mutations require an effective owner with a fresh credential. |
...
Use `owners.emails` in `towk.toml` as the recovery path. The email must be verified before it makes the user an effective owner.

## DM privacy

DMs use the same permission names where they make sense, but Towk keeps a hard privacy boundary around private conversations. Moderation and room-administration permissions such as `message.manage`, `message.echo`, `room.manage`, `room.list`, and `room.create` are denied in DMs regardless of role grants.
...
| ------------------- | ----- |
| Set recovery owners | `owners.emails` in `towk.toml` |
...
<Aside type="caution">
  Treat `owners.emails` as the lockout recovery path. Towk does not expose an operator command to wipe and reseed RBAC state in normal deployments.
</Aside>

<LinkCard
  title="Security & Privacy"
  href="/guides/operations/security/"
  description="Authentication, encryption, the privacy boundary, and how Towk handles secrets."
/>
