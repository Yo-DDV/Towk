...
Towk supports local password accounts, email verification and reset flows, external OAuth/OIDC providers, explicit account linking, and owner recovery through verified email addresses.
...
port = 587
tls = "mandatory"
username = "towk@example.com"
password = "<generate-me>"
from = "Towk <towk@example.com>"
```
...
CHATTO_SMTP_USERNAME=towk@example.com
CHATTO_SMTP_PASSWORD=<generate-me>
CHATTO_SMTP_FROM="Towk <towk@example.com>"
```
...
Towk does not silently merge accounts by email. External identities link by verified issuer or provider namespace plus provider subject.

| Flow | Behavior |
| ---- | -------- |
| New external identity with `auto_provision = true` | User confirms account creation before Towk creates the account. |
...
| Disconnecting a provider | Towk refuses to remove the last sign-in method from a passwordless account. |

This avoids accidental account takeover through recycled or unverified email addresses.

## Owner Recovery

Set owner recovery emails in `towk.toml`:
...
The email must be verified by the user before Towk treats the user as an effective owner. Owners are recovery-grade: they virtually receive every normal permission and cannot be locked out through ordinary RBAC denies.
...
Towk uses opaque runtime credentials, not JWTs. Browser sessions use signed HTTP-only cookies for same-origin flows and bearer tokens for registered cross-origin server connections. Password resets, password changes, external identity disconnects, and account deletion revoke older credentials through the user's auth generation.
