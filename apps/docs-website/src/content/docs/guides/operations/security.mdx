...
There is **no per-IP or per-user rate limiting** at the application layer. If you expose Towk on the public internet, a request-rate limiter (e.g. Caddy `rate_limit`, Cloudflare, or an upstream WAF) is recommended.
...
OAuth redirect callbacks use a separate trust list, `webserver.oauth_redirect_origins`. See [Towk frontend OAuth redirects](#chatto-frontend-oauth-redirects).
