...
description: Configure OIDC, GitHub, GitLab, Google, or Discord sign-in for a Towk server.
---

import { Aside, Tabs, TabItem } from "@astrojs/starlight/components";

Towk can consume external OAuth and OIDC providers for sign-in. Providers are configured in `towk.toml` as repeated `[[auth.providers]]` tables, and the login page renders one button per configured provider.

<Aside type="note">
  Towk acts as an OAuth/OIDC client here. This does not change Towk's own OAuth authorization endpoint used by Towk frontends when they connect to a server.
</Aside>
...
Towk derives the callback from `webserver.url`, so set that value to the public HTTPS origin of your server.

For older OIDC client registrations, Towk also serves `/auth/oidc` and `/auth/oidc/callback` as compatibility aliases for the configured OIDC provider. New registrations should use the provider-specific callback URL.
...
`issuer_url` must be an absolute `http` or `https` URL. Towk uses OIDC discovery from that issuer and verifies the ID token before matching the external identity.

By default, Towk does not request email claims from OIDC providers:
...
Email-less accounts can sign in, link identities, and count toward `limits.max_users` through the linked OIDC subject. They do not receive a Towk verified email unless the provider later returns an explicitly verified email claim.
...
```bash
CHATTO_AUTH_PROVIDERS_0_ID=towk-hub
CHATTO_AUTH_PROVIDERS_0_TYPE=oidc
CHATTO_AUTH_PROVIDERS_0_LABEL="Towk Hub"
...
Any `CHATTO_AUTH_PROVIDERS_<index>_*` variables replace providers from `towk.toml`.

Towk also accepts the older single-provider OIDC variables for compatibility:

```bash
CHATTO_AUTH_OIDC_ENABLED=true
CHATTO_AUTH_OIDC_ISSUER_URL=https://id.example.com
CHATTO_AUTH_OIDC_CLIENT_ID=...
CHATTO_AUTH_OIDC_CLIENT_SECRET=...
# CHATTO_AUTH_OIDC_LABEL="Towk Hub"
```
...
By default, Towk requests the minimum provider scopes needed to identify the
...
For OIDC providers, Towk always includes `openid` even if you omit it from a custom scope list.

`request_email` defaults to `false`. Set `request_email = true` to request provider email scopes. For OIDC, the default request is `openid profile`; Towk can create, link, and sign in accounts from the provider `sub` without receiving an email claim.
...
Towk matches external logins by provider identity, not by email address.
...
If a provider callback returns an identity that is not linked to an account, Towk does not silently pick an account by email.
...
Existing users can link providers from their account settings. Linking requires the user to already be signed in, lets Towk create a short-lived provider link-start URL, and asks the user to confirm the pending link after the provider callback. Pending create and link tokens are short-lived and scoped to that specific action.
...
A Towk verified email is stricter. Towk creates one from a provider email only when the provider adapter has an explicit verification signal:

| Provider | When Towk stores a verified email |
| -------- | ----------------------------------- |
...
| `gitlab` | Not currently promoted to a Towk verified email |
...
Provider IDs stay provider-specific, so an OIDC provider can use an ID such as `towk-hub` while reporting its provider `type` as `oidc`.
