...
By default, Towk stores persisted assets (attachments, avatars, server branding images, and link-preview images) inside NATS JetStream object stores. This works well for small to medium deployments, but can put pressure on your NATS storage as usage grows.
...
The `SERVER_ASSETS` JetStream quota still covers temporary upload chunks while S3 is enabled. Its 10 GB default is not an S3 capacity limit: configure lifecycle, capacity alerts, and a provider-side quota for the bucket independently.
...
- **S3 prefixes are movable** — Towk stores prefix-free asset keys, so you can move objects to a different S3 prefix and update config without rewriting Towk metadata
...
CHATTO_CORE_ASSETS_S3_BUCKET=towk-assets
CHATTO_CORE_ASSETS_S3_PATH_PREFIX=production/towk
...
Towk creates the bucket automatically on startup if it doesn't exist.
...
Any service that implements the S3 API works with Towk. Towk uses path-style URLs by default for custom S3-compatible endpoints, which works with providers that don't support virtual-hosted bucket URLs.
...
You can also change `path_prefix` after moving or copying the S3 objects to the new prefix. Towk stores logical, prefix-free asset keys in NATS, so no Towk metadata rewrite is needed.

<Aside type="caution">
  You **cannot** change from one S3 provider or bucket to another. Towk only has a single S3 configuration — if you switch endpoints or buckets, files stored in the previous location become inaccessible. If you need to migrate between S3 providers, copy the data at the storage level first.
</Aside>
