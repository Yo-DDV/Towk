...
| `client_instance_id` | `string` | Opaque browser-session identifier used to make retries idempotent. Empty is accepted for compatibility with older clients. |
| `mode` | [`JoinCallMode`](/reference/connectrpc-api/types/#chatto-api-v1-JoinCallMode) | Requested behavior when another device for the account is active. |
| `expected_call_id` | `string` | Optional. Exact active call advertised by a notification action. When it no longer matches, the request fails instead of starting a replacement. |
...
| `status` | [`JoinCallStatus`](/reference/connectrpc-api/types/#chatto-api-v1-JoinCallStatus) | Structured join outcome for multi-device clients. |
| `participant_id` | `string` | Media-session participant ID when joined. |
| `device_index` | `uint32` | Stable display slot when joined. |
| `active_device_count` | `uint32` | Number of other active connections for this account at decision time. |
| `companion_allowed` | `bool` | Whether another companion connection can be admitted without a transfer. |
| `call_id` | `string` | Exact call session that admitted this connection. |
...
| `client_instance_id` | `string` | Must match the browser-session identifier admitted by JoinCall. Empty is accepted for compatibility with older clients. |
| `expected_call_id` | `string` | Optional. Exact call selected by a notification action. A mismatch fails instead of issuing credentials for a replacement call. |
...
| `participant_id` | `string` | Media-session participant ID encoded as the LiveKit identity. |
| `device_index` | `uint32` | Stable display slot for this connection. |
...
| `client_instance_id` | `string` | Browser-session identifier for the exact connection leaving the call. Empty targets the legacy single-device participant only. |
| `expected_call_id` | `string` | Optional. Exact call being left. A mismatch is an idempotent no-op so a delayed request cannot remove the same browser from a replacement call. |
