...
| `thread_root_event_id` | `optional string` | Thread root when the cursor belongs to a thread timeline. |
| `user_id` | `string` | User whose public cursor advanced. |
| `event_id` | `string` | Highest message event covered by the advance. |
| `event_sequence` | `uint64` | Monotonic room-stream sequence for event_id. |
| `read_at` | `google.protobuf.Timestamp` | Server-recorded time of the advance. |
