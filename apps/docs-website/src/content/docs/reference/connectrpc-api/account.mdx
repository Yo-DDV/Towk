...
Updates the authenticated user's login, display name, and/or biography.
...
| `biography_markdown` | `optional string` | Markdown biography. Empty clears the biography. Supported clients and the core enforce at most 1,024 Unicode code points and 4 KiB of UTF-8. Raw HTML and remote images are not rendered by supported clients. |
...
| `show_last_activity` | `optional bool` | Whether other authenticated server members may see last activity. Absent stored state is treated as enabled for upgrade compatibility. |
| `read_receipts_enabled` | `optional bool` | Enables or disables reciprocal public read receipts for this account. |
...
| Field | Type | Description |
| --- | --- | --- |
| `status` | [`PresenceStatus`](/reference/connectrpc-api/types/#chatto-api-v1-PresenceStatus) | Live status requested for the authenticated user. Offline is accepted only for an installation-scoped release. |
...
| `installation_id` | `string` | Stable browser-installation identifier shared with push and realtime state. New clients send it with every session-aware report. |
| `session_id` | `string` | Opaque page-lifetime identifier. Multiple tabs from one installation own independent leases so one tab cannot incorrectly hide another active tab. |
| `active` | `bool` | Whether this exact page is currently active after lifecycle grace periods. |
| `meaningful_activity` | `bool` | True only for foreground/resume or direct user interaction. Routine lease refreshes leave this false and cannot advance profile latest activity. |
| `release_installation` | `bool` | Withdraw every session lease owned by installation_id. This is used by "Look offline" and requires status Offline with an empty session_id. |
...
| Field | Type | Description |
| --- | --- | --- |
| `status` | [`PresenceStatus`](/reference/connectrpc-api/types/#chatto-api-v1-PresenceStatus) | Effective user-level status after aggregating all current installations. |
...
| Field | Type | Description |
| --- | --- | --- |
| `current_password` | `string` | Current password used only when the runtime credential is no longer fresh. |
