...
// requireNonOwnerAdministrativeTarget prevents delegated administration from
// targeting an effective owner. Trusted system maintenance and ordinary
// self-service paths remain outside this delegated boundary.
func (c *ChattoCore) requireNonOwnerAdministrativeTarget(ctx context.Context, actorID, targetUserID string) error {
	if actorID == SystemActorID || actorID == targetUserID {
		return nil
	}
	isOwner, err := c.IsServerOwner(ctx, targetUserID)
	if err != nil {
		return err
	}
	if isOwner {
...
func (c *ChattoCore) AdminListUsers(ctx context.Context, search string, limit, offset int) (*AdminUserList, error) {
	...
	if offset < 0 { return nil, ErrInvalidArgument }

	search = strings.TrimSpace(search)
	if strings.Contains(search, "@") {
		user, err := c.GetUserByVerifiedEmail(ctx, search)
		if errors.Is(err, ErrNotFound) { return &AdminUserList{}, nil }
		if err != nil { return nil, err }
		if offset > 0 { return &AdminUserList{TotalCount: 1}, nil }
		view, err := c.AdminGetUser(ctx, user.GetId())
		if err != nil { return nil, err }
		return &AdminUserList{Users: []*AdminUserView{view}, TotalCount: 1}, nil
	}
	...
	return &AdminUserList{Users: users, TotalCount: totalCount, HasMore: offset+len(users) < totalCount}, nil
}
...
func (c *ChattoCore) AdminGetUserByLogin(ctx context.Context, login string) (*AdminUserView, error) {
	...
	if err != nil { return nil, fmt.Errorf("user not found: %w", err) }
...
func (c *ChattoCore) AdminCreateUserAs(ctx context.Context, actorID string, req AdminCreateUserRequest) (*AdminUserView, error) {
	...
	if displayName == "" { displayName = strings.TrimSpace(req.Login) }
	if email := strings.TrimSpace(req.VerifiedEmail); email != "" {
		claimed, err := c.IsEmailClaimed(ctx, email)
		if err != nil { return nil, err }
		if claimed { return nil, ErrEmailAlreadyVerified }
	}
	for _, roleName := range req.RoleNames {
		if roleName == RoleEveryone { return nil, ErrImplicitRole }
		if !c.RBAC.RoleExists(roleName) { return nil, ErrRoleNotFound }
	}
	...
	if err != nil { c.rollbackUserCreation(ctx, user); return nil, err }
	return adminUser, nil
}

func (c *ChattoCore) AdminUpdateOperatorUser(ctx context.Context, req AdminUpdateOperatorUserRequest) (*AdminUserView, error) {
	if req.Login == nil && req.DisplayName == nil { return nil, ErrInvalidArgument }
...
func (c *ChattoCore) AdminSetUserPasswordAs(ctx context.Context, actorID, userID, password string) (*AdminUserView, error) {
	if err := c.requireNonOwnerAdministrativeTarget(ctx, actorID, userID); err != nil { return nil, err }
	if err := c.SetPasswordHashAs(ctx, actorID, userID, password); err != nil { return nil, err }
...
func (c *ChattoCore) AdminDeleteUserAs(ctx context.Context, actorID, userID string) error {
	if err := c.requireNonOwnerAdministrativeTarget(ctx, actorID, userID); err != nil { return err }
...
func (c *ChattoCore) AdminAddUserVerifiedEmailAs(ctx context.Context, actorID, userID, email string) (*AdminUserView, error) {
	if err := c.requireNonOwnerAdministrativeTarget(ctx, actorID, userID); err != nil { return nil, err }
	if err := c.AddVerifiedEmailDirectAs(ctx, actorID, userID, email); err != nil { return nil, err }
	return c.AdminGetUser(ctx, userID)
}

func (c *ChattoCore) AdminAssignUserRole(ctx context.Context, userID, roleName string) (*AdminUserView, error) {
	if err := c.AssignServerRoleToExistingUser(ctx, SystemActorID, userID, roleName); err != nil { return nil, err }
	return c.AdminGetUser(ctx, userID)
}

func (c *ChattoCore) AdminRevokeUserRole(ctx context.Context, userID, roleName string) (*AdminUserView, error) {
	if err := c.RevokeServerRoleFromExistingUser(ctx, SystemActorID, userID, roleName); err != nil { return nil, err }
	return c.AdminGetUser(ctx, userID)
}
