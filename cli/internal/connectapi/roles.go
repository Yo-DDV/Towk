...
func (s *publicRoleService) GetRole(ctx context.Context, req *connect.Request[apiv1.GetRoleRequest]) (*connect.Response[apiv1.GetRoleResponse], error) {
	if _, err := requireCaller(ctx); err != nil { return nil, err }
	if req.Msg.GetName() == "" { return nil, invalidArgument("name is required") }
...
func (s *publicRoleService) BatchGetRoles(ctx context.Context, req *connect.Request[apiv1.BatchGetRolesRequest]) (*connect.Response[apiv1.BatchGetRolesResponse], error) {
	...
	for _, name := range req.Msg.GetNames() {
		...
		if err != nil {
			if errors.Is(err, core.ErrRoleNotFound) { continue }
...
func (s *roleService) requireRoleAdminRead(ctx context.Context, userID string) error {
	canManage, err := s.api.core.CanManageRoles(ctx, userID)
	if err != nil { return err }
	canAssign, err := s.api.core.CanAssignRoles(ctx, userID)
	if err != nil { return err }
	canManageUsers, err := s.api.core.CanManageUserPermissions(ctx, userID)
	if err != nil { return err }
	if !canManage && !canAssign && !canManageUsers { return core.ErrPermissionDenied }
	return nil
}
...
func (s *roleService) ListRoles(ctx context.Context, _ *connect.Request[adminv1.ListRolesRequest]) (*connect.Response[adminv1.ListRolesResponse], error) {
	...
	return connect.NewResponse(&adminv1.ListRolesResponse{
		Roles: adminAPIRoles(catalog.Roles), ViewerCanManageRoles: catalog.ViewerCanManageRoles,
...
func (s *roleService) GetRole(ctx context.Context, req *connect.Request[adminv1.GetRoleRequest]) (*connect.Response[adminv1.GetRoleResponse], error) {
	...
	return connect.NewResponse(&adminv1.GetRoleResponse{
		Role: adminAPIRole(details.Role), Users: s.apiRoleUsers(ctx, details.Users),
...
func (s *roleService) DeleteRole(ctx context.Context, req *connect.Request[adminv1.DeleteRoleRequest]) (*connect.Response[adminv1.DeleteRoleResponse], error) {
	caller, err := requireCaller(ctx)
	if err != nil { return nil, err }
	if err := s.api.core.AdminDeleteServerRole(ctx, caller.UserID, req.Msg.GetName()); err != nil { return nil, connectError(err) }
...
func publicAPIRoles(roles []core.RoleWithPermissions) []*apiv1.Role {
	...
	for i := range roles { out = append(out, publicAPIRole(&roles[i])) }
	return out
}
...
func publicAPIRole(role *core.RoleWithPermissions) *apiv1.Role {
	...
	return &apiv1.Role{
		...
		IsSystem: core.IsDefaultSystemGrade(role.Name), Position: role.Position,
		Pingable: role.Pingable, Color: role.Color,
	}
}

func adminAPIRoles(roles []core.RoleWithPermissions) []*adminv1.AdminRole {
	out := make([]*adminv1.AdminRole, 0, len(roles))
	for i := range roles { out = append(out, adminAPIRole(&roles[i])) }
	return out
}
...
func adminAPIRole(role *core.RoleWithPermissions) *adminv1.AdminRole {
	...
	return &adminv1.AdminRole{
		Role: publicAPIRole(role), Permissions: corePermissionsToStrings(role.Permissions),
...
func (s *roleService) apiRoleUsers(ctx context.Context, users []core.RoleUserSummary) []*apiv1.User {
	...
	for _, user := range users {
		...
		if err != nil { presence = core.PresenceStatusOffline }
		out = append(out, &apiv1.User{
			Id: user.ID, Login: user.Login, DisplayName: user.DisplayName, Deleted: user.Deleted,
			PresenceStatus: corePresenceStatusToAPI(presence), CustomStatus: coreCustomStatusToAPI(user.CustomStatus),
		})
	}
	return out
}
