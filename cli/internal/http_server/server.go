...
func limitRequestBody(maxBytes int64, readTimeout time.Duration) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if c.Request.ContentLength > maxBytes {
			writeLocalizedError(c, http.StatusRequestEntityTooLarge, "http.request_body_too_large")
			c.Abort()
			return
		}
...
func (s *HTTPServer) setupRoutes() error {
	...
	if len(encKey) > 0 {
		...
		s.logger.Warn("webserver.cookie_encryption_secret is not set; session cookies are signed but NOT encrypted. Run `towk init` on a fresh server to generate one, or add a hex-encoded 32-byte value to towk.toml.")
