...
This is **not** a tutorial, design doc, or API reference. If a concept needs more than a paragraph, link to the relevant [FDR](fdr/INDEX.md), [ADR](adr/INDEX.md), [ARCHITECTURE.md](ARCHITECTURE.md), or [CONTRIBUTING.md](../CONTRIBUTING.md) rather than inlining.

Entries within each section are ordered by **conceptual flow** — foundational terms first, derivatives after — not alphabetically.
...
**Pane Header** — The top bar of a content pane (Room View, settings page, admin page, etc.). Carries the title, optional subtitle, optional back arrow, and icon-only action buttons via the `actions` snippet. Chunky labelled buttons belong in the body, not here.
...
**Panel** — Bordered card used across instance-admin (`/chat/[serverId]/admin/*`) and per-server settings pages. Shared visual chrome for administrative interfaces.
...
**Scope** — Tier at which a permission is configured: `server`, `group`, or `room`. For non-owners, all applicable user and role decisions across valid scopes contribute; any deny wins, otherwise any allow grants, otherwise the API treats the result as denied. See [FDR-001](fdr/FDR-001-roles-and-permissions.md).
...
**Subject** — NATS message topic. Current durable facts use `evt.{aggregateType}.{aggregateId}.{eventType}`; transient sync uses `live.sync.…`; committed EVT facts are internally republished on `live.evt.…`. See [ARCHITECTURE.md](ARCHITECTURE.md#evt-subject-patterns).
...
**Runtime credential family** — One bearer credential or a chain of rotated cookie handles that shares a non-renewable absolute lifetime. See [FDR-023](fdr/FDR-023-authentication-and-sessions.md) and [ADR-046](adr/ADR-046-typed-runtime-credentials.md).
...
**Republish** — JetStream feature that mirrors accepted stream messages onto another NATS subject. Towk uses it to expose committed EVT facts on `live.evt.>`; `myEvents` treats that as an internal feed, not a client contract. See [ARCHITECTURE.md](ARCHITECTURE.md#current-resources).
