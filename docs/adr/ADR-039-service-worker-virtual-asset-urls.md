...
**Update (2026-07-05):** Towk no longer uses Service Worker virtual asset
...
- Media `Range` requests are not cached by the asset proxy. The worker redirects them to the hidden target URL so browser media playback keeps current Range behavior until Towk has deliberate Range streaming through the proxy path.
...
- **Copied DOM URLs are no longer access tickets in the main app path.** A copied `/__chatto/assets/...` URL is same-origin and only useful inside a controlled Towk browser session with the matching registered server and hidden ticket mapping.
...
- **Backend asset serving now has a proxy mode.** `X-Chatto-Asset-Proxy: 1` is a private contract between the browser Service Worker and Towk servers. It must be allowed by CORS and included in `Vary` so proxy-streamed responses do not mix with presigned-redirect responses.
