...
Towk's frontend uses Svelte 5 runes (`$state`, `$derived`) for reactive state management. The room event stores receive realtime events and projected timeline refreshes, then cache renderable events for child components to consume.
