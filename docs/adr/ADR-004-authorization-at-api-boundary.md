...
- **Risk of forgetting authorization checks**: A new GraphQL resolver that calls core without checking permissions would bypass operation-specific authorization. Mitigated by code review, resolver helper conventions, and the current authorization model in [FDR-001](../fdr/FDR-001-roles-and-permissions.md).
