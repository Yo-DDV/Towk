...
- **Projections are derived state.** Each aggregate type has one or more projections — in-memory Go data structures rebuilt from the event stream — that serve reads. Projections live entirely in process memory; multiple Towk processes each maintain their own copies, consuming the same stream.
...
- **No third-party framework dependency.** Trade: we own the abstractions and their evolution. Read both `looplab/eventhorizon` and `blinkinglight/bee` for design vocabulary, but ship our own minimal package shaped to Towk's needs.
