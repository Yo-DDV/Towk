...
- **Proto changes are breaking changes**: Modifying proto definitions affects wire format compatibility. Changes to `proto/` are treated as significant compatibility work according to [ADR-045](ADR-045-public-api-stability-tiers.md) and [CONTRIBUTING.md](../../CONTRIBUTING.md).
- **Not human-readable**: Debugging stream contents requires `protoc --decode_raw` or similar tooling.
