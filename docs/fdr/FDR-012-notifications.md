...
**Decision:** Towk's static web manifest prefers `minimal-ui` and falls back directly to `browser`, without a `standalone` fallback, for installed-web contexts. When the server receives an Android Chromium manifest request, it forces `display` and `display_override` to `browser` so Chrome opens Towk in a normal browser surface instead of a WebAPK/standalone surface that can create the URL-copy foreground notification. Towk does not attempt to dismiss or spoof that fixed browser-owned notification. If an existing Android install still launches in Chrome's legacy standalone mode, Towk detects that runtime state and shows a blocking localized in-app migration notice with a user-triggered Chrome Android intent link. The link navigates the current installed context instead of opening a second `_blank` window, so users are not left inside the old foreground container after choosing the Chrome path.
**Why:** That notification is generated by the browser/OS, not by Towk's service worker, Web Push sender, or notification store. Treating it as an app notification would lead to ineffective changes in the wrong layer.
**Tradeoff:** Android Chromium users lose the standalone-looking installed window and get normal browser chrome instead. Existing Android WebAPKs can still require user-driven reinstall or Chrome's own WebAPK manifest-update cycle before Chrome applies the browser display mode. A Teams-like Android notification model with no browser-owned foreground disclosure and native app chrome requires a packaged mobile distribution strategy, not only a web manifest tweak.
