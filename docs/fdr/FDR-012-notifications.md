...
**Decision:** Towk's web manifest prefers `minimal-ui` and falls back directly to `browser`, without a `standalone` fallback, for installed-web contexts. Towk does not attempt to dismiss or spoof the fixed Android Chrome foreground notification that can expose a "copy current URL" action for installed standalone PWAs. If an existing Android install still launches in Chrome's legacy standalone mode, Towk detects that runtime state and shows a blocking localized in-app migration notice with a user-triggered Chrome Android intent link. The link navigates the current installed context instead of opening a second `_blank` window, so users are not left inside the old foreground container after choosing the Chrome path.
...
**Tradeoff:** Browsers that cannot use `minimal-ui` may show Towk in a normal browser surface instead of a standalone installed-app surface. Already-created Android WebAPKs can require user-driven reinstall or Chrome's own WebAPK manifest-update cycle before Chrome applies the new app mode. A Teams-like Android notification model with no browser-owned foreground disclosure requires a packaged mobile distribution strategy, not only a web manifest tweak.
