...
**Tradeoff:** Browsers that cannot use `minimal-ui` may show Towk in a normal browser surface instead of a standalone installed-app surface. Already-created Android WebAPKs can require user-driven reinstall before Chrome applies the new app mode. A Teams-like Android notification model with no browser-owned foreground disclosure requires a packaged mobile distribution strategy, not only a web manifest tweak.
