...
**Tradeoff:** Android Chromium users may see the minimal browser controls that come with `minimal-ui`. Existing Android WebAPKs can still require user-driven uninstall/reinstall or Chrome's own WebAPK manifest-update cycle before Chrome applies the new identity and display mode. A Teams-like Android notification model with no browser-owned foreground disclosure and native app chrome requires a packaged mobile distribution strategy, not only a web manifest tweak.
