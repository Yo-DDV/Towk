...
**Decision:** Towk's static web manifest prefers `minimal-ui` and falls back directly to `browser`, without a `standalone` fallback, for non-Android-Chromium installed-web contexts. Android Chromium manifest responses now use `display: browser` plus a dedicated Android install `id` so new installs do not reuse a stale standalone/WebAPK identity from earlier builds. The service worker does not cache `/manifest.webmanifest`, because the response is intentionally browser-specific. Towk does not attempt to dismiss or spoof Chrome's fixed browser-owned URL-copy notification, and it does not block the app with a runtime modal when the browser reports `display-mode: standalone`.
...
**Tradeoff:** Android Chromium users keep Chrome's normal browser surface instead of a fully app-like WebAPK window. This is the only pure-web manifest path that avoids Chrome's own foreground URL-copy disclosure on affected Android builds; a native-app-style Android notification model with no browser-owned foreground disclosure and native app chrome requires a packaged mobile distribution strategy such as TWA/Capacitor/native, not only a web manifest tweak. Existing Android WebAPKs can still require user-driven uninstall/reinstall or Chrome's own manifest-update cycle before Chrome applies the new identity and display mode.
