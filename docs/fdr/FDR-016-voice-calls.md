...
- Every joined screen-share tile exposes an always-visible diagnostics action. The sender and each receiver can opt in to a local panel showing delivered resolution, frame rate, media bitrate, packet loss, WebRTC transport and codec details, sender congestion limits or receiver playback health, plus bounded 60-second trend charts.
...
### 12. Screen-share diagnostics are opt-in and browser-local
