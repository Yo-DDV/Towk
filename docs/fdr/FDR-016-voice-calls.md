...
- Camera and screen-share tiles expose feed-fullscreen and local-mute controls on hover-capable devices, and keep those controls visible on touch devices. Feed fullscreen is local to the viewer's browser: Towk uses the native Fullscreen API when available and falls back to a safe-area-aware full-viewport viewer when the browser does not expose it, including iPhone WebKit. Remote participant mute is also local to the viewer and does not change server state or other participants' audio. Local participant tiles show the same mute affordance, wired to the viewer's own microphone mute.
