...
- **Audit log page** — chronological diagnostic event-log view for forensic review, grouped by event creation date. The list view uses `AdminEventLogService.ListEvents`; the detail view uses `AdminEventLogService.GetEvent` to show a diagnostic JSON projection for human inspection. Credential material such as password hashes is always redacted; the underlying durable EVT event remains complete for replay.
...
**Decision:** Raw storage subjects, stream/consumer names, redacted payload JSON, projection metric names, and memory estimates are documented as diagnostic values. The admin diagnostics APIs are intentional operator APIs, but clients should not parse those raw values as stable product-domain data. Diagnostic event payloads omit credential material even when the durable replay event must retain it.
