...
- On deletion, the server first shreds every discoverable KMS wrapping key, then purges the app-owned DEK records from `RUNTIME_STATE`, and finally records one `UserKeyShreddedEvent` on the user aggregate. The ordering preserves alternate wrapping-key references until all KEKs are gone. Missing key records are treated as an interrupted retry, and user-aggregate OCC makes the durable shred event idempotent across replicas.
- Account deletion fails closed if crypto-shredding or its durable event cannot complete: the account-deleted fact, profile cleanup, asset cleanup, and credential revocation do not run until a retry has completed the shred boundary. Once that boundary succeeds, the server removes profile data, deletes the avatar, records durable deletion facts for message-owned assets and derivatives, and revokes sessions and bearer tokens. An elected cleanup worker retries physical removal for current message-owned asset deletion facts.
...
**Decision:** Account deletion shreds the KMS wrapping-key refs before removing the app-owned DEK records, then appends exactly one durable `UserKeyShreddedEvent` under user-aggregate OCC. Encrypted message bodies and durable user PII stay on disk but become permanently unreadable; projections use the shred event to tombstone authored messages before attempting decryption. A retry tolerates already-missing key records and completes the durable signal. Message-owned assets, including derivative children such as thumbnails and video variants, receive `AssetDeletedEvent` and have their backing bytes removed only after the shred boundary succeeds.
**Why:** Scanning every JetStream stream and KV bucket for a user's messages would be slow, error-prone, and leave fragments in backups and replicas. Destroying the wrapping keys makes the text unreadable, while the shred event gives projections and cleanup code a deterministic audit signal. Preserving DEK records until every discoverable KEK is gone keeps alternate stored wrapping refs available after a partial attempt. Backups specifically exclude the encryption key bucket so that restoring a backup doesn't restore the ability to read deleted users' messages. See ADR-007.
**Tradeoff:** Crypto-shredding spans KMS, KV, and EVT rather than one atomic transaction. The operation is therefore ordered and idempotent, and account deletion stops on failure until a caller retries. Encrypted-but-unreadable message bytes linger forever. Storage cost is small for text; binary assets are explicitly deleted because signed URLs could otherwise keep serving blobs until expiry.
...
**Why:** A clean KMS boundary is what makes future extraction to Vault / AWS KMS / HSM possible without turning the external KMS into Towk's DEK registry. Keeping wrapped DEKs in `RUNTIME_STATE` also preserves them in normal data backups without backing up the KEKs needed to unwrap them. See ADR-007.
