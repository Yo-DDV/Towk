...
**Decision:** Clients choose a supported recorder format with `MediaRecorder.isTypeSupported`, preferring MP4/AAC when the browser can record it and falling back to WebM/Opus or Ogg/Opus where needed. The server accepts first-class voice metadata only for `audio/webm`, `audio/mp4`, or `audio/ogg`, validates duration and 24–96 finite waveform peaks, verifies the corresponding WebM, ISO-BMFF, or Ogg container signature, and normalizes non-MP4 voice uploads into an `audio/mp4` M4A/AAC asset before making the attachment durable.
