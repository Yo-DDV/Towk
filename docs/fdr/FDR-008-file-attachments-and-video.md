...
- Default upload size limits: 25 MB for every file, including original videos when video processing is disabled; enabling video processing gives videos a separate 100 MB default limit. Server-side video and animated-GIF processing also rejects sources above the configured processing envelope: 20 minutes and 3840×2160 display pixels by default. Non-GIF video sources without detectable duration or display dimensions fail processing instead of consuming a worker until timeout; animated GIFs keep the dedicated bounded fallback described below.
...
- Ordinary audio attachments render with the browser's native audio player plus an explicit download action. Cross-platform audio playback is guaranteed only for first-class voice messages, which are normalized separately; arbitrary uploaded audio files remain dependent on browser and operating-system codec support.
- Raw video playback is reserved for deployments where video processing is disabled. When video processing is enabled and no processing manifest exists yet, clients wait in the portable-variant state for every user video source instead of opening the raw original in the embedded player. Container labels such as MP4 or WebM are still treated as hints, not as a guarantee that every viewer can decode the source codec. When processing is disabled, the raw original remains available and playback depends on the viewer's browser and operating system codec support.
...
**Decision:** Transcoding produces multiple H.264 MP4 variants whose target resolutions are derived from the source display resolution. A 1080p source might yield 720p and 480p; a 480p source skips the higher tiers. Processing metadata records display dimensions so clients can reserve the correct frame for sources with non-square pixels or rotation metadata. Generated thumbnails are rendered at display dimensions with square pixels. Before thumbnail or variant generation starts, the probed source must have trustworthy duration and display dimensions and must remain inside the configured processing envelope (`video.max_duration`, `video.max_pixels`) so file size is not the only DoS boundary. Animated GIFs are the exception for missing duration: the processor skips slow format-duration probing for GIF inputs and caps the demuxer to the probed stream duration or a 30-second fallback. The chat timeline treats ambiguous near-square landscape video metadata as a widescreen presentation case for ordinary uploaded videos.
...
**Tradeoff:** The access ticket is a bearer capability — anyone holding it can fetch until the expiry passes or the signed user loses room membership. `core.AssetAccessTicketTTL` is currently **24 hours** so normal rendering, lazy loading, deferred media startup, and lightbox use are reliable across long-lived room views; clients still refresh URL fields through projected timeline, message, or attachment-list reads when tickets approach expiry or a media load fails. Streamed protected responses use `private, no-cache` with an `ETag`: a browser may retain bytes, but Towk validates the current ticket, asset state, and room membership before it can return `304`. The Service Worker does not persist those bodies. Heavy passive S3 originals use short-lived redirects whose object response remains `private, no-store`. Rotating `[core.assets].signing_secret` invalidates all outstanding access tickets.
...
**Decision:** Towk creates or updates `SERVER_ASSETS` with `MaxBytes`, defaulting to 10 GB. This quota covers NATS-backed persisted assets and temporary upload chunks. Before a NATS-backed completion writes its durable object, Towk revalidates the final byte size after image processing or voice normalization and atomically resizes that upload's reservation; a larger generated result cannot bypass concurrent capacity accounting. It still applies to temporary chunks when completed assets use S3, but it does not cap the external S3 bucket. `LINK_PREVIEW_ASSETS` keeps its independent 1 GB default quota. Existing objects are retained if an operator lowers the limit below current usage; JetStream rejects subsequent writes until usage falls below the configured bound.
